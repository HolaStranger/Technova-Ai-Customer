...
// WARRANTY SERVICE
// ==============================
app.post("/warranty/check", (req, res) => {
  try {
    const { purchaseDate } = req.body;

    if (!purchaseDate) {
      return res.status(400).json({ error: "purchaseDate required" });
    }
    ...
    const isValid = diffDays <= 730; // 2 years rule

    res.json({
      warrantyStatus: isValid ? "VALID" : "EXPIRED",
      daysUsed: Math.floor(diffDays),
    });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
